...
class MCTS(object):
    ...
    def _playout(self, env, agent, termination=False, truncation=False):
        ...
        action_probs, leaf_value = self._policy(env, agent)
    ...
    def get_move_probs(self, env, agent, temp=1e-3):
...
class MCTSPlayer(object):
    ...
    def get_action(self, env, agent, temp=1e-3, return_prob=0):
        # observation, reward, termination, truncation, info = board.last()
        # mask = observation["action_mask"]
        sensible_moves = list(env.env.env.env.board.legal_moves)  # TODO 이게 맞음?
